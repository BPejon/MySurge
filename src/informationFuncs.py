...
# ============================================================================
# CORPUS INDEX AND REFERENCE EXTRACTION FUNCTIONS
# ============================================================================

# Variáveis globais para cache do corpus
_corpus_cache = None
_corpus_index = None

def load_corpus_index(corpus_path="dataMatScience/corpusMatScience.json"):
    """
    Carrega um índice do corpus mapeando títulos para doc_ids.
    ...
    try:
        ...
        if Path(corpus_path).exists():
            with open(corpus_path, 'r', encoding='utf-8') as f:
                ...
                _corpus_index = {item['Title'].lower(): item['doc_id'] for item in corpus}
                return _corpus_index
        
        # Se não encontrar, tenta caminho absoluto a partir do workspace
...
def is_journal_format(reference_text):
    """
    Detecta se a referência está no formato: 
    [num] Autores. Título. *Revista*, publicação
    
    Retorna True se seguir este padrão (tem [num] e pelo menos 2 pontos antes do asterisco).
    """
    # Padrão: [número] ... . ... . *
    # Deve ter [número], depois ponto (fim dos autores), depois mais texto com ponto, depois asterisco
    pattern = r'\[\d+\].*?\. .+?\. \*'
    return bool(re.search(pattern, reference_text))

def extract_title_from_reference(reference_text):
    """
    Extrai o título de uma referência no formato:
    [num] Autores. Título. *Revista*, publicação
    
    Usa regex para capturar diretamente o título.
    Remove quebras de linha e espaços extras.
    
    Retorna apenas o título, sem formatação.
    """
    try:
        # Padrão: "and [Sobrenome]. TÍTULO. *"
        # Captura tudo entre "and [Sobrenome]." e o próximo ". *"
        pattern = r'and\s+\w+\.\s+(.+?)\.\s*\*'
        ...
        if match:
            ...
            # Remove o último sobrenome se estiver no começo (ex: "Adebayo. A rock...")
            # Padrão: se começa com [Letra]+. [Maiúscula], remove a primeira parte
            cleanup = re.sub(r'^[A-Z][a-z]*\.\s+', '', captured)
            
            return cleanup if cleanup else captured
...
def find_title_in_corpus(title, corpus_path="dataMatScience/corpusMatScience.json"):
    """
    Busca um título no corpus. Retorna o doc_id se encontrar, None caso contrário.
    
    Usa busca exata primeiro (case-insensitive), depois busca similar se necessário.
    """
    ...
    if title_lower in index:
        return index[title_lower]
    
    # Busca similar (para casos com pequenas diferenças)
    best_match = None
    best_ratio = 0.85  # Threshold de similaridade (85%)
    
    for corpus_title, doc_id in index.items():
        ratio = SequenceMatcher(None, title_lower, corpus_title).ratio()
        if ratio > best_ratio:
            best_ratio = ratio
            best_match = doc_id
...
def extract_references_improved(text, corpus_path="dataMatScience/corpusMatScience.json"):
    """
    Versão melhorada de extract_references que detecta o formato da referência.
    
    Para referências no formato journal ([num] Autores. Título. *Revista*...):
    - Extrai o título corretamente
    - Busca no corpus e retorna: (ref_num, "num:doc_id") se encontrar
    - Caso contrário retorna: (ref_num, "num:título")
    
    Para outros formatos: usa o método antigo (extrai sentença).
    """
    pattern = r'\[(\d+)\]'
    ...
    for match in matches:
        ...
        # Extrai a referência completa (até o final da linha ou próxima referência)
        ref_end = text.find('\n', start_pos)
        if ref_end == -1:
            next_ref_match = re.search(r'\n\[\d+\]', text[start_pos + 10:])
            ref_end = len(text) if next_ref_match is None else start_pos + 10 + next_ref_match.start()
        
        reference_text = text[start_pos:ref_end]
        
        # Verifica se é formato de journal
        if is_journal_format(reference_text):
            # Extrai o título
            title = extract_title_from_reference(reference_text)
            
            if title:
                # Busca no corpus
                doc_id = find_title_in_corpus(title, corpus_path)
                
                if doc_id is not None:
                    # Encontrou no corpus - retorna ID
                    results.append((ref_number, f"{ref_number}:{doc_id}"))
                else:
                    # Não encontrou no corpus - retorna título
                    results.append((ref_number, f"{ref_number}:{title}"))
            else:
                # Não conseguiu extrair o título, usa método antigo
            ...
            # Não é formato de journal, usa método antigo
            sentence_start = text.rfind('.', 0, start_pos) + 1
            sentence_end = text.find('.', start_pos)
            if sentence_end == -1:
                sentence_end = len(text)
